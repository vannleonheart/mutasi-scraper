...
class ScraperBank {
    ...
    try {
      ...
      try {
        if (this.page.$(".ns-box-inner p") !== null) {
          const message = await this.page.$eval(
            ".ns-box-inner p",
            (el) => el.innerText
          );
      ...
      await this.page.goto(
        "https://ibank.bankmandiri.co.id/retail3/loginfo/logout"
      );
      await this.browser.close();
      let arr = [];
  ...
  async getBCA(tglawal, blnawal, tglakhir, blnakhir) {
    ...
    try {
      ...
      const reg = result
        .split("Saldo")[1]
        .split(
          '</table>  </td></tr><tr>  <td colspan="2">    <table border="0" width="70%" cellpadding="0" cellspacing="0" bordercolor="#ffffff">'
        )[0];
  ...
  async getDanamon() {
    ...
    try {
      ...
      await page.waitForSelector(
        "#frmDefault > div > div.transaction-area > div.ld-menu"
      );
      await page.goto(
        "https://www.danamonline.com/onlinebanking/default.aspx?usercontrol=DepositAcct/dp_TrxHistory_new"
      );
      ...
      await page.waitForTimeout(2000);
      await page.goto(
        "https://www.danamonline.com/onlinebanking/Login/lgn_logout.aspx"
      );
      await browser.close();
  ...
  async getBRI(norek) {
    ...
    try {
      ...
      try {
        await page
          .waitForSelector('img[class="alignimg"]')
          .then(() => console.log("LOG : all page loaded"));
      ...
      const recog = await axios.post(
        "https://www.nyckel.com/v0.9/functions/yduzh0xptiwz8ans/ocr",
        form,
        {
          headers: {
            ...form.getHeaders(),
      ...
      await page.waitForSelector(
        "#wrapper > div.header-wrap > div > div.logoib.col-1-2 > img"
      );
      ...
      await page.waitForNavigation();
      if (
        (await page.$('a[id="myaccounts"]')) !== null ||
        (await page.$('a[href="Logout.html"]')) !== null
      ) {
        ...
        try {
          ...
          await frame
            .waitForSelector('a[href="AccountStatement.html"]')
            .then(() => console.log("LOG : all page loaded for mutasi menu"));
          console.log(
            "LOG : Loading ajax progress after click accountStatement menu..."
          );
        ...
        try {
          const frame = await page
            .frames()
            .find((fr) => fr.name() === "content");
          await frame
            .waitForSelector("#ACCOUNT_NO")
            .then(() => console.log("LOG : all page loaded"));
        ...
        try {
          const frame = await page
            ...
            .find((fr) => fr.name() === "content");
          await frame
            .waitForSelector("#divToPrint > div > table")
            .then(() => console.log("LOG : all page loaded"));
          const DataMutasi = await frame.evaluate(() => {
            const rekeningData = document.querySelectorAll(
              "#divToPrint > div > table > tbody > tr > td:nth-child(2)"
            );
          ...
          await page.click(
            "#main-page > div.headerwrap > div > div.uppernav.col-1-2 > span:nth-child(1) > a:nth-child(4) > b"
          );
          await page.waitForTimeout();
          console.log("LOG : Logout account from ibri");
          page.on("dialog", async (dialog) => {
